					...
					if ($noOfHotelBookings[0] > 0) : ?>
						...
						<div class="col-sm-12 ticketTableContainer pullABitLeft" id="hotelBookingsWrapper">
							...
							<table class="table table-responsive">
								...
								<?php
								//checking hotel booking query and join table with booking
								// SELECT * FROM hotelbookings JOIN hotels ON hotelbookings.hotelName = hotels.hotelID WHERE hotelbookings.username="kodega" AND hotelbookings.cancelled="no"; 
								// $hotelBooksSQL = "SELECT * FROM `hotelbookings` WHERE username='$user' AND cancelled='no' JOIN `hotels` ON hotelbookings.bookingID=bookings.bookingID";
								// $hotelBooksSQL = "SELECT * FROM `hotelbookings` WHERE username='$user' AND cancelled='no' JOIN `hotels` ON hotelbookings.bookingID=bookings.bookingID";
								// SELECT hotelbookings.*, hotels.hotelName AS hotel FROM `hotelbookings` JOIN `hotels` ON hotelbookings.hotelName = hotels.hotelID WHERE username = 'kodega' AND cancelled = 'no';
								$hotelBooksSQL = "SELECT hotelbookings.*, hotels.hotelName AS hotel FROM hotelbookings JOIN hotels ON hotelbookings.hotelName = hotels.hotelID WHERE hotelbookings.username='$user' AND hotelbookings.cancelled='no'";
								// SELECT * FROM hotelbookings JOIN hotels ON hotelbookings.hotelName = hotels.hotelID LIMIT 0, 25; 
								...
								?>
									...
									<tr>
										...
										<td class="tableElementTagsNoHover text-center"><?php echo $hotelBooksRow["hotel"]; ?></td>
